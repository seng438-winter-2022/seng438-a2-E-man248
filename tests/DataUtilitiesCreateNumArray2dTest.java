...
{
	//Test Case 1 ('Null' data passed in)
	@Test
	public void createNumberArray2D_NullTest() 
	{		
		//put it in try-catch to catch any exceptions
			try
	        {
				//no need to initialize an object of DataUtilities because 'createNumberArray2D(double[][] data)' is a static method
	        ...
	        {
	        	//If anything other than an InvalidParameterException is detected go into this loop
	            if (!(e instanceof InvalidParameterException))
	            {
	            	//Print out what error happened if not the expected exception
	                System.out.println("Assertion Error: Expected Invalid Parameter Exception, got " + e.toString());
	                //Since we do not get the InvalidParameterException we want, this is a fail
	                fail();
	            }
